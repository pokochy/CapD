...
from models.schemas import ScanJob, ScanJobOut, ScanJobSummary, ScanResult, ScanStatus
from utils.reporter import generate_html_from_job
...
async def get_report_html(job_id: str, db: AsyncSession = Depends(get_db)):
    """특정 스캔의 HTML 보고서 (브라우저에서 바로 확인 가능)."""
    stmt = (
        select(ScanJob)
        .where(ScanJob.id == job_id)
        .options(selectinload(ScanJob.results))
    ...
    if not job:
        raise HTTPException(status_code=404, detail="Report not found")
    html = generate_html_from_job(job, fingerprint=job.fingerprint_result)
